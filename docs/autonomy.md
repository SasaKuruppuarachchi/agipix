...
The functionality of each autonomy module included in this framework in alphabetical order:
 ...
 - `trajectory_planner`: The trajectory planning library for autonomous robots. [details](https://github.com/SasaKuruppuarachchi/agipix_px4_autonomy/tree/main/trajectory_planner)
...
Prerequisite [Software setup]({{ '/docs/software-setup.html' | relative_url }})
...
### Autonomous Exploration: Exploring an unknown environments and create a map.

```bash
# start simulator
ros2 launch uav_simulator start_launch.py # recommend to use the floorplan2 env for your first trial
...
### Autonomous Inspection: Navigating to the target and inspecting it with a zig-zag path.

```bash
# start simulator
ros2 launch uav_simulator start_launch.py # # recommend to use the tunnel_dynamic_1 env for your first trial
