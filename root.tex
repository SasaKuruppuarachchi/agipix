...
% \usepackage{cite} % duplicate, removed
\usepackage{tabularx}
\usepackage{cancel}
\usepackage{bm}
\usepackage{url}
\usepackage{mathtools}
% \usepackage{textcomp} % duplicate, removed
\usepackage{nccmath}
\usepackage{xcolor}
\usepackage{multirow}
\usepackage{hhline}
...
\begin{figure*}[t]
        \centering
        \includegraphics[width=0.96\textwidth]{images/diag/autonomy_heuristcs.png}
        \caption{Integrated hardware and software architecture of Agipix. The PX4 FCU handles low-level control while the Jetson Orin NX runs ROS~2 containers for perception, mapping, planning, and mission management. Native DDS over serial (micro XRCE-DDS) provides direct topic bridging without protocol translation.}
...
\begin{figure}[t]
        \centering
        \includegraphics[width=0.95\linewidth]{images/real.jpg}
        \caption{Agipix hardware prototype. Annotated sensor positions and dimensions (438~\(\times\)~372~mm envelope).}
...
ROS~2 (DDS) underpins all inter-process communication. On the companion computer, a Micro XRCE-DDS Agent runs alongside the autonomy containers and exposes the PX4 autopilot topics natively in ROS~2 over a UART transport. The PX4 autopilot publishes actuator status, odometry, battery, and IMU streams as ROS~2 topics; command topics (trajectory setpoints, attitude/thrust targets) flow in the reverse direction with bounded latency. This direct DDS integration eliminates MAVLink/MAVROS translation and reduces serialization overhead. QoS is tuned per stream (e.g., IMU: BEST\_EFFORT/KEEP\_LAST, LiDAR: RELIABLE, maps: TRANSIENT\_LOCAL). A simulation bridge in Isaac Sim mirrors the exact interface (topic names and QoS), publishing synthetic LiDAR, camera, and IMU with noise models (Table~\ref{tab:noise_models}), enabling identical launch descriptions in simulation and hardware.
\subsection{Perception and State Estimation}
We employ Direct LiDAR-Inertial Odometry (DLIO) as the baseline LIO backend, featuring a coarse-to-fine continuous-time trajectory for motion correction and lightweight computation \cite{chen2022dlio}. Inputs are a deskewed LiDAR point cloud and a 6-axis IMU; strict LiDAR–IMU time synchronization is enforced. When visual texture permits, camera constraints can be fused for map enrichment; otherwise the pipeline falls back to LiDAR-only. The LIO publishes odometry for downstream mapping and planning and supports on-demand map export to PCD for evaluation reproducibility.
\subsection{Mapping and Exploration}
Online mapping maintains voxel TSDF and occupancy layers derived from LIO poses and LiDAR depth. Frontier detection operates on the occupancy layer to identify candidate exploration goals. An information-gain objective selects the next-best-view subject to clearance, flight envelope, and battery constraints. The mapping module exposes persistent map topics (TRANSIENT\_LOCAL) for late-joining consumers and supports semantic overlays as an optional future extension.
\subsection{Planning and Control}
The autonomy stack comprises modular packages: a global planner for waypoint/frontier selection, a trajectory planner (time-parameterized polynomials or sampling-based) with a time optimizer, and a tracking controller. Plans are generated against the online map (clearance and dynamic updates) and respect thrust and body-rate limits. The controller publishes position/velocity or attitude/thrust setpoints as ROS~2 topics consumed directly by PX4 via the Micro XRCE-DDS Agent, matching the same interface in simulation and on the real platform.

\subsection{Operator Interface}
Three operator workflows are supported: (i) Safety RC as the primary manual failsafe, (ii) manual flight via UDP-MAVLink from a ground station with a USB joystick, and (iii) ROS~2 velocity teleoperation over DDS using a joystick. During autonomous exploration, the UI visualizes live point clouds/maps, robot state, and mission progress, and exposes pause/resume and goal selection. Human-in-the-loop autonomy allows high-level intents (waypoints/regions) to be converted into trajectories with explainable overlays and smooth handover between autonomy and manual control.
\begin{figure}[t]
        \centering
        \includegraphics[width=0.95\linewidth]{images/diag/sim_overview.png}
        \caption{Simulation-to-reality workflow: Isaac Sim digital twin generates synchronized sensor streams passed through the ROS~2 bridge; identical perception and planning containers run in sim and real flights for zero-shot transfer.}
        \label{fig:sim_pipeline}
\end{figure}

\section{SIMULATION AND DIGITAL TWIN}
\subsection{Simulation Environment and Workflow}
Host requirements are Ubuntu 20.04/22.04 with an NVIDIA GPU; Isaac Sim (ROS~2 bridge autoload) provides photorealistic rendering and sensor simulation. Optional Nucleus enables asset sharing. Pegasus can be enabled as an Isaac Sim extension for flight scenario tooling. The typical workflow is: (i) launch Isaac Sim with the ROS~2 bridge, (ii) start the Micro XRCE-DDS Agent and autonomy containers (tmux layout), (iii) run PX4 SITL when applicable with \texttt{UXRCE\_DDS\_SYNCT=false} to align timestamps, and (iv) set \texttt{use\_sim\_time=true} in ROS~2 nodes. Visualization and introspection use Foxglove via \texttt{ws://localhost:8765}.
...
\subsection{Reproducibility and Configuration Parity}
We enforce sim-to-real parity via: (i) pinned container images (reporting image digests), (ii) shared launch/compose files and QoS YAML across sim and hardware branches, (iii) a deterministic tmux orchestration, and (iv) strict time synchronization (PX4 parameterization and ROS~2 \texttt{use\_sim\_time}). These constraints reduce drift between environments and make experiments repeatable.
...
\begin{itemize}
        ...
        \item Reproducibility: container image digest checks and launch determinism (hash and QoS parity).
\end{itemize}
\subsection{Experimental Protocol}
Define scenario classes aligned with indoor exploration: narrow corridor traversal, multi-room exploration, and mixed lighting. Execute each in simulation first, then on the real platform using identical mission scripts and configuration files. Record drift, coverage, latency (with Foxglove timelines), and CPU/GPU utilization.
...
\begin{figure}[t]
        \centering
        \includegraphics[width=0.9\linewidth]{images/diag/ui_overview.png}
        \caption{Operator-facing UI overview used during teleoperation and exploration.}
